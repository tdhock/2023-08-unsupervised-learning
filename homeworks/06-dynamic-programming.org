Homework 6: Dynamic programming for optimal changepoint detection
...
2. Run jointseg::Fpsn which is a fast log-linear dynamic programming
   ...
   (loss values) and t.est (changepoints). t.est is a matrix with one
   row per model size, and one column per segment end position. So for
   example row 5 of t.est has 5 numbers, each of which is the position
   of a segment end, when there are 5 segments total (4
   changepoints). Also run binsegRcpp::binseg_normal for
   comparison. Use a max of 20 segments for both algorithms. Plot
   y=loss as a function of x=segments using different colors in a
   single geom_line for different algorithms (e.g., binseg=red,
   DP=black). Does DP have a lower loss as expected?
...
4. code the dynamic programming algorithm from scratch in an R function DYNPROG, using the pseudo-code in the article as reference. 
