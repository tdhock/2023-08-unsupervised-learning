...
cat(type.colors.latex, "
\\usepackage{amsmath}
\\DeclareMathOperator*{\\argmin}{arg\\, min}
", file="08-optimal-segmentation-colors.tex", sep="\n")
...
get_segs_loss <- function(pid, chrom, max.segments=6){
  ...
  binseg <- suppressWarnings(binsegRcpp::binseg_normal(logratio, max.segments))
  ...
  segs.dt <- rbind(
    ...
    data.table(
      algorithm=a("binseg"),
      coef(binseg)[, .(segments, start, end, mean)]))
  one[, data.i := .I]
...
```{r fig.height=8}
suppressWarnings(gg+coord_cartesian(xlim=c(0, 300)))
```
...
```{r fig.height=8}
suppressWarnings(gg+coord_cartesian(xlim=c(2100, 2300)))
```
...
```{r fig.height=8}
suppressWarnings(gg+coord_cartesian(xlim=c(3700, 3800)))
```
...
- Statistical model: for every segment $s\in\{1,\dots,S\}$,
    $x_i \stackrel{\text{iid}}{\sim} N(u_s, \sigma^2)$ for every
    data point $i\in(t_{s-1},t_s]$ implies square loss function
    $\ell(u_s, x_i)=(u_s-x_i)^2$ to minimize.
...
t_1<\cdots<t_{S-2}
  ...
  \sum_{s=1}^{S-1}\  \sum_{i=t_{s-1}+1}^{t_s} \ell( u_s,  x_i)
}_{
\mathcal L_{S-1, t_{S-1}}
} +
\underbrace{\min_{u_S} \sum_{i=t_{S-1}+1}^{t_S=n} \ell( u_S,  x_i)}_{
c_{(t_{S-1}, t_S=n]}
}
...
- Hard optimization problem because of integer-valued changepoint $t_s$
  variables, naively $O(n^{S-1})$ time.
- Auger and Lawrence (1989): $O(Sn^2)$ time classical dynamic
  programming algorithm: best loss computed recursively, 
  for all $s\in\{1,\dots,S\}$ and for all $t\in\{S,\dots,n\}$:
$$
\mathcal T^*_{s,t},\mathcal L_{s,t}=
\argmin,\min_{t'<t} \mathcal L_{s-1,t'} + c_{(t', t]}
$$
...
$$
c_{(0, t]} = \min_u \sum_{i=1}^t (x_i-u)^2 =
\sum_{i=1}^t x_i^2 - 2\hat u_t\sum_{i=1}^t x_i + t\hat u_t^2.
$$

We can use cumulative sum to compute $y_t$ for all $t$ in linear
$O(t)$ time,
$y_t = \sum_{i=1}^t x_i.$
...
$z_t = \sum_{i=1}^t x_i^2.$

And best loss up to $t$, $c_{(0,t]}=$
$$ 
\sum_{i=1}^t x_i^2 - 2\hat u_t\sum_{i=1}^t x_i + t\hat u_t^2 =
z_t - 2(y_t/t)(y_t) + t (y_t/t)^2 = 
z_t-y_t^2/t.
$$
...
```{=latex}
\begin{tabular}{ccccc}
Segments & naive & DP(classic) & DP(pruned) & BinSeg \\
\hline
1 & $O(n)$   & $O(n)$   & $O(n)$--$O(n^2)$ & $O(n)$ \\
2 & $O(n)$   & $O(n^2)$ & $O(n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
3 & $O(n^2)$ & $O(n^2)$ & $O(n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
4 & $O(n^3)$ & $O(n^2)$ & $O(n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
...
```{r, dev='tikz', results="asis"}
showChoice(234, 3, 157)
```

---

# Comparison with binary segmentation

```{r fig.height=8} 
models <- get_segs_loss("4", "2")
gg <- ggplot()+
  theme.mid+
  geom_point(aes(
    data.i, logratio),
...
At the same time we compute an entry of the $\mathcal L_{s,t}$ matrix
(min loss), we must also compute an entry of the $\mathcal T^*_{s,t}$
matrix (argmin, best last change in $s$ segments up to data point
$t$).

Then we use backtracking/decoding to find the best change-point
positions for a given model size. For example in previous slides we
saw,

* $\mathcal T^*_{4,234} = 157$
* $\mathcal T^*_{3,157} = 113$
* $\mathcal T^*_{2,113} = 41$
* which are the best changepoints for model size 4.
