...
ggplot()+
  geom_point(aes(
    V1, V3, color=factor(y)),
    data=xy.dt)

ggplot()+
  geom_point(aes(
    V4, V5, color=factor(y)),
    data=xy.dt)
...
candidate_loss <- function(indices){
  ...
  for(feature in 1:N.col){
    ...
    split.loss.list[[feature]] <- data.table(
      feature,
      split.index=seq_along(split.point), split.point,
      loss.split,
      loss.constant)
  }
...
new_node <- function(indices){
  ...
  node$split <- function(){
    ...
    for(child.name in names(logical.list)){
      ...
      node[[child.name]] <- new_node(child.indices)
    }
    c(node$lo, node$hi)
  }
  node$expr <- function(){
...
for(iteration in 1:5){
  ...
  , by=node.i])
  split.i <- term.best[which.min(loss.split-loss.constant), node.i]
...
ggplot()+
  facet_grid(. ~ iteration, labeller=label_both)+
  geom_tile(aes(
    V1, V2, fill=pred),
    data=grid.pred)+
  geom_point(aes(
...
ggplot()+
  geom_point(aes(
    ...
    shape=1,
    data=viz.dt)+
  facet_grid(. ~ node.i + feature, labeller=label_both)
...
node.loss.list <- list()
for(node.i in seq_along(index.list)){
  node.i.loss <- candidate_loss(index.list[[node.i]])
  node.loss.list[[node.i]] <- data.table(node.i, node.i.loss)
}
(node.loss <- rbindlist(node.loss.list))
