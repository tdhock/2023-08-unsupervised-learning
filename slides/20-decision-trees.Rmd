...
candidate_loss <- function(indices){
  ...
  for(feature in 1:N.col){
    x.vec <- X.mat[,feature]
    ord.indices <- order(x.vec)
    x.ord <- x.vec[ord.indices]
    y.cum <- c(0,cumsum(y.vec[ord.indices]))
...
dtree <- new.env()
dtree$indices <- 1:N.row
split.loss <- candidate_loss(dtree$indices)
ggplot()+
  geom_point(aes(
    split.index, loss.split),
    shape=1,
    data=split.loss)+
  facet_grid(. ~ feature)
ggplot()+
  geom_point(aes(
    split.point, loss.split),
    shape=1,
    data=split.loss)+
  facet_grid(. ~ feature)
best.loss <- split.loss[which.min(loss.split)]
