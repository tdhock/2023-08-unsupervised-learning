...
let $f(j,e)=\min,\argmin_{t\in j,\dots,e} D_{j,e}(t)$ be the best loss difference/split on segment $(j,e)$.

Initialization. Let $\mathcal L_1=L_{1,n}$ be the loss with one segment, 
let $d_1,t_1=f(1,n)$ be the first loss difference/split,
and let $\mathcal S_1=\{(1,n,d_1,t_1)\}$ be the initial segment to split.

Recursion. For all $k\in\{2,\dots,K\}$: ($K\leq n$ is max segments)

* $j^*_k, e^*_k, d^*_k, t^*_k = \argmin_{(j,e,d,t)\in\mathcal S_{k-1}} d$ (best segment to split)
* $\mathcal L_k=\mathcal L_{k-1} +d^*_k$, (loss)
* $\underline d_k,\underline t_k=f(j^*_k,t^*_k)$ and $\overline d_k,\overline t_k=f(t^*_k+1,e^*_k)$ (new splits)
* $\mathcal N_k=\{
(j^*_k, t^*_k, \underline d_k, \underline t_k),
(t^*_k+1,e^*_k,\overline d_k,\overline t_k)
\}$ (new segments)
* $\mathcal V_k=\{(j,e,d,t)\in\mathcal N_k\mid e-j\geq 1\}$ (splittable segments)
* $\mathcal S_k=
...
* $f(j,e)=\min,\argmin_{t\in\{j+\ell-1,\dots,e-\ell\}} D_{j,e}(t)$ (best split)
* $\mathcal V_k=\{(j,e,d,t)\in\mathcal N_k\mid e-j+1\geq 2\ell\}
\}$ (splittable segments)
