...
each $(j,e)\in\mathcal S$ represented by a segment start $j$ and end
$e$ (both in $1,\dots,n$).

Then the next segment to split, and best split point, are the solution
to the following problem,

$$ \min_{(j,e)\in\mathcal S} \min_{t\in\{j,\dots,e\}} 
L_{j,t} + L_{t+1,e} - L_{j,e}. $$

Use the cumsum trick to compute loss in constant time,
$$\sum_{i=j}^t x_i = \sum_{i=1}^t x_i - \sum_{i=1}^{j-1} x_i = y_{t+1} - y_j.$$
$$L_{j,t} = z_{t+1}-z_j - (y_{t+1}-y_j)/(t-j+1).$$
